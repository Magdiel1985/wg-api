...
add_user() {
    ...
    echo "$user $_VPN_IP $public_key" >> ${SAVED_FILE} && echo -e "\e[44m[wg-api cli]\e[0m Created $user"
}
...
suspend_user() {
    ...
    read ip key <<<"$(awk "/^$user /{print \$2, \$3}" ${SAVED_FILE})"
    if [[ -n "$key" ]]; then
        wg set $interface peer $key allowed-ips 127.0.0.1/32
        if [[ $? -ne 0 ]]; then
            echo "wg set failed"
            exit 1
        fi
...
unsuspend_user() {
    ...
    read ip key <<<"$(awk "/^$user /{print \$2, \$3}" ${SAVED_FILE})"
    if [[ -n "$key" ]]; then
        wg set $interface peer $key allowed-ips 0.0.0.0/0
        if [[ $? -ne 0 ]]; then
            echo "wg set failed"
            exit 1
        fi
...
if [[ $action == "-i" ]]; then
    ...
    generate_cidr_ip_file_if
elif [[ ! -z "$user" && ( $action == "-a" || $action == "-d" || $action == "-s" || $action == "-u" ) ]]; then
    do_user
else
    usage
    exit 1
