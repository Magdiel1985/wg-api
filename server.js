...
fastify.get('/peer/suspend/:username', function(request, reply) {
    cliMsg(`${request.ip} requested suspend of peer ${request.params.username}`)
    if (!authAction(request)) return
    exec(`bash ./scripts/bash/wg.sh -s ${request.params.username}`, (err, stdout, stderr) => {
        reply.send(JSON.stringify({ code: 200, profile: "Suspended" }, null, 2))
    });
})

fastify.get('/peer/unsuspend/:username', function(request, reply) {
    cliMsg(`${request.ip} requested reactivation of peer ${request.params.username}`)
    if (!authAction(request)) return
    exec(`bash ./scripts/bash/wg.sh -u ${request.params.username}`, (err, stdout, stderr) => {
        reply.send(JSON.stringify({ code: 200, profile: "Unsuspended" }, null, 2))
    });
})
...
fastify.get('/peer/key/:username/wg0.conf', function(request, reply) {
    cliMsg(`${request.ip} requested plaintext cert of peer ${request.params.username}`)
    if (!authAction(request)) return
    try {
        reply.sendFile(path.join("profiles", request.params.username, "wg0.conf"))
